...
void usage()
{
  fprintf(stderr,"Usage: cfitav sourcefile\n");
  fprintf(stderr,"  Parse cfitav sourcefile\n");
  exit(1);
}
...
int main(int argc, char *argv[])
{
  ...
  cfitavbuf = loadsource(s);

  if (!cfitavbuf) usage();

  ast = skpparse(cfitavbuf,program,trc);
